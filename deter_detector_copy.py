...
async def get_transaction_urls(session, fir_block, las_block=None):
    ...
    return coros
        #tasks.append(asyncio.create_task(check_all_transactions(session, transactions, fir_block, block)))
    
    #return [item for lst in total_tx for item in lst]
...
async def async_manager(blocks):
    async with aiohttp.ClientSession(trust_env=True, connector=aiohttp.TCPConnector()) as session:
        ...
        print('finished getting num of pages:')
        print(txs_n_tasks[0])
        total_txs = []
        new_tasks = []
...
async def check_transaction(session, url, curr_block, og_block, tx):
    ...
    print(f'at {tx}, found these actions{actions}')
    #checks for arbitrage and if there is it returns a string
    if check_arbitrage(actions):
        return f'arbitrage in transaction: {tx}, at block: {curr_block}, around empty block: {og_block}'
...
async def async_manager2(data):
    async with aiohttp.ClientSession(trust_env=True) as session:
        ...
        for piece in data:
            url, curr_block, og_block, tx = piece.split('|')
            tx = tx[:-1]
    ...
    return [attack for attack in results if attack]
...
def get_potential_EVM(start_value=0, file_name='transactions.txt', increment_val=50):
    file1 = open('deter_attacks.txt', 'r')

    #reads all the lines of the files and puts them in a list
    blocks = file1.readlines()
    ...
    for i in range(start_value, len(blocks), increment_val):
        ...
        try:
            ...
            for tx, block, og_block in txs:
                file2.write(f'{ETHERSCAN_TX}{tx}|{block}|{og_block}|{tx}\n')
            file2.close()
...
def get_EVM(start_val=0, file_name='arbitrage.txt', increment_val=200):
    reading_file = open('transactions.txt')

    urls = reading_file.readlines()

    #asyncio.run(async_manager2(urls[0:1]))
    for i in range(start_val, len(urls), increment_val):
        check_time = time.time()
        writing_file = open(file_name, 'a')
        ...
        try:
            ...
            for attack in attacks:
                writing_file.write(attack + '\n')

            time.sleep(random.randint(5,10))
        except Exception as e:
            traceback.print_exc()
            writing_file.write(f'ended at: {i}')
            break
        writing_file.close()
...
print(manager.proxies)
get_EVM(start_val=53400, increment_val=400)
