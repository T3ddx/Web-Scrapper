...
class RequestGenerator:
    ...
    async def check_page(self, session, proxy): 
        ...
        while(True):
            try:
                ...
                temp = await session.get('https://etherscan.io', proxy=f'http://{proxy}', ssl=False, timeout=self.timeout)
                
                content = await temp.content.read()

                if len(BeautifulSoup(content, 'html.parser').select('header.header.border-bottom.d-print-none nav div a.navbar-brand')) == 0:
                    raise Exception
    ...
    async def get_proxy(self):
        ...
        if not self.refreshing and len(self.proxies) <= 8:
...
# IDEAS
# 1. If a proxy doesn't work, use my IP - check
# 2. Make it so proxies have to error 5 times on 3 different occassions - check
#       - Use a hashmap to do this
#       - The key to this change is in ban_proxies function
# 3. For some reason, after first refresh, no more connections are made - ???
#       - Could have something to do with the TCPConnector limit
#       - Go back to waiting to have an empty proxy list before refreshing
# 4. incrementing by 25 blocks might be the key - poss not
#       - 100 connections at once at any time
# 5. 
        ...
        while(True):
            for _ in range(5):
                try:
                    ...
                    if BeautifulSoup(content, 'html.parser').select('h1.h5.mb-0')[0].text != 'Transaction Details ':
                        raise Exception
                    return content
                except Exception:
                    pass
...
# async def retest_proxies(session, proxy):
#     i=0
#     rerun = True
#     while(rerun):
#         try:
#             resp = await session.get(f'https://etherscan.io/', headers=manager.create_headers(), ssl=False, proxy=f'http://{proxy}')
#             rerun = False
#             print(f'proxy: {proxy} - status: {resp.status}')
#             return proxy
...
#             tasks.append(asyncio.create_task(retest_proxies(session, proxy)))
#         print(await asyncio.gather(*tasks))

# manager = RequestGenerator()
